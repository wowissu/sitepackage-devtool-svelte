...
import type { GamePlatformItem } from '@/src-vscode/providers/items/game/game.platform.item';

export default class PlatformPanel {
  public static readonly ViewType = 'platform.panel';
  ...
  constructor(public readonly contextItem: GamePlatformItem, public readonly panel: vscode.WebviewPanel, public readonly app: App) {
    this._message();
    this._update();
  ...
  private async _message() {
    ...
    const commands = {
      [MsgEnum.PlatformOnMount]: () => this._postgame(),
      [MsgEnum.PlatformOnError]: ({ text }) => {
        vscode.window.showErrorMessage(text);
      },
      [MsgEnum.PlatformOnSubmit]: async ({ game }) => {
        await this.contextItem.write(game);
        await this._postgame();
        vscode.window.showInformationMessage(`${game.id} is updated.`);
      },
      [MsgEnum.PlatformShowSaveDialog]: async () => {
        try {
  ...
  private _postgame() {
    ...
    return webview.postMessage({
      command: MsgEnum.PlatformPostGame,
      game: this.game,
  ...
  private async _getHtml(webview: vscode.Webview): Promise<string> {
    try {
      const publicUri = vscode.Uri.joinPath(PlatformPanel.App.ExtensionUri, 'public');
  ...
  public static use(app: App) {
    ...
    PlatformPanel.App = app;
    PlatformPanel.ExtContext = extContext;

    extContext.subscriptions.push(
      vscode.commands.registerCommand(App.Commands.OpenPlatformWebview, (contextItem: GamePlatformItem) => {
        PlatformPanel.createOrShow(contextItem, app);
      })
    );
  }
  ...
  public static extContextUri(...p: string[]) {
    return vscode.Uri.joinPath(PlatformPanel.App.ExtensionUri, ...p);
  }
   ...
   * @param app
   */
  public static createOrShow(contextItem: GamePlatformItem, app: App) {
    ...
    new PlatformPanel(contextItem, panel, app);
  }
}
